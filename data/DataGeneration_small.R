...
# Use this script to generate data with m = 10, but with different sample size n. To compare the objective of CD and optimal solutions.
...
weighted_adjs[[1]] <- weighted_adj(graphs[[1]]$dag)
...
for (i in 1:length(m_list)) {
  writeLines(sapply(random_interventions, function(x) paste(x[[i]]$targets, collapse = ",")), paste0("./SyntheticData/Small/","graph",i,"/intervention_targets.txt"))
}
...
for (n_idx in 1:length(n_list)) {
  for (env in 1:n.environment) {
    data_int <- interventional_data_generate(weighted_adjs[[m_idx]], random_interventions[[env]][[m_idx]]$targets, n_list[n_idx], n.iter)
    path_i =  paste0("./SyntheticData/Small", "/graph", m_idx, "/environment", env)
    if (!dir.exists(path_i)) {
      dir.create(path_i, recursive = TRUE)
    }
    for (iter in 1:n.iter) {
      write.table(data_int$X[[iter]], paste0(path_i, "/data_n_", n_list[n_idx], "_iter_",iter, ".csv"), row.names=FALSE, col.names=FALSE, sep=",")
    }
  }
}
# observational data
for (n_idx in 1:length(n_list)) {
  ...
  path_i =  paste0("./SyntheticData/Small/", "/graph", m_idx, "/observational")
  if (!dir.exists(path_i)) {
    dir.create(path_i, recursive = TRUE)
  }
  for (iter in 1:n.iter) {
