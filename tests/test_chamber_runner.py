...
class ChamberRunnerTests(unittest.TestCase):
    ...
    def job(self, method="igsp_complete"):
        return next(j for j in self.design["jobs"] if j["method"] == method)

    def result(self, method="igsp_complete", optimal=True):
        job = self.job(method)
    ...
    def test_fixed_design_and_canonical_integer_float_budget(self):
        d = self.design
        self.assertEqual(len(d["jobs"]), 97)
        self.assertEqual(list(d["configurations"]), ["scm_4"])
        self.assertEqual([sum(j["method"] == m for j in d["jobs"]) for m in study.METHODS], [17,17,17,8,15,15,8])
        self.assertEqual(len({j["output"] for j in d["jobs"]}), 97)
    ...
    def test_scientific_inputs_unchanged_from_old_five_minute_run(self):
        ...
        for before, after in zip(old["jobs"], self.design["jobs"]):
            for field in ("index", "method", "setting", "data_sha256", "target_information", "output"):
    ...
    def test_dry_run_is_read_only_and_uses_selected_method(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with redirect_stdout(out), mock.patch.object(runner, "execute") as execute:
                self.assertEqual(runner.main(["--dry-run", "--methods", "gies_complete", "--setting", "15",
                    "--time-limit", "3600", "--output-root", str(root)]), 0)
            self.assertFalse(root.exists())
            execute.assert_not_called()
            report = json.loads(out.getvalue())
            self.assertEqual((report["planned"], report["selected"]), (97, 1))
    ...
    def test_baselines_use_one_hour_algorithm_timer(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            timer.assert_called_once_with(3600, "igsp_complete")
    ...
    def test_export_refuses_incomplete_and_extra_fragments(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            data.write_json(root / "scm_5/parts/igsp_complete/setting_01.json", {})
    ...
    def test_complete_but_failed_run_exports_and_returns_failure(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(report["status_counts"]["process_failed"], 97)
