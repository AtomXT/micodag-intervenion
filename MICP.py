...
def optimization(data, moral, l):
    ...
    delta_opt = []
    for e in range(1, n_environment):
        delta_opt.append([1 if abs(deltas[e][i].x) > 0.5 else 0 for i in range(p)])
    return Gamma_opt, delta_opt, model.MIPGAP, model.ObjVal, end-start
    ...
    for graph in [2]:
        ...
        # lmbda = 0.1
            ...
            for iter in range(1, 11):
                ...
                approach = 'rank'
                lmbda_gnies = 6
                gnies_estimate = gnies.fit(data_gnies, approach=approach, lmbda=lmbda_gnies)
                end = time.time()
            ...
            gnies_rank_results.to_csv(f'./Results/estimations/gnies_rank_graph{graph}_lambda{lmbda_gnies}_time.csv')
